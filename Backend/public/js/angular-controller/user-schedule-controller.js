system.controller("UserScheduleController", UserScheduleController);
/**
 *
 * @param {type} $scope
 * @param {type} $http
 ...
 * @returns {undefined}
 */
function UserScheduleController($scope, $http, $rootScope, $timeout, $interval, Upload) {
    this.__proto__ = BaseController($scope, $http, $rootScope);
    $scope.controllerName = "UserScheduleController";
    $scope.filter = {
        pageId: 0,
        pageSize: 20
    };
    ...
    $scope.getSchedule = () => {
        var url = '/api/user_schedule?sorts=-created_at&embeds=user,images,videos&page_id=' + $scope.filter.pageId + '&page_size=' + $scope.filter.pageSize;
        params = buildFilter();
        if (params.length > 0) {
    ...
    $scope.updateSchedule = (scheduleId) => {
        ...
        $http.patch('/api/user_schedule/' + scheduleId, data).then(response => {
            if (response.data.status == 'successful') {
    ...
    $scope.create = () => {
        ...
        $http.post('/api/user_schedule', data).then(response => {
            if (response.data.status == 'successful') {
    ...
    $scope.update = () => {
        ...
        $http.patch('/api/user_schedule/' + $scope.schedule.id, data).then(response => {
            if (response.data.status == 'successful') {
    ...
    $scope.delete = (item) => {
        ...
        $http.delete('/api/user_schedule/' + item.id).then(response => {
            if (response.data.status == 'successful') {
